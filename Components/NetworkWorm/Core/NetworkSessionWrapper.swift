        ...
        case .data:
            return self.urlSession.dataTask(with: request, completionHandler: { (data, response, error) in
                ...
                if let theResponse = response {
                    let item = String(describing: theResponse)
                    print("response -> \(item)")
                }
                ...
                else if let httpResponse = (response as? HTTPURLResponse), self.isError(statusCode: httpResponse.statusCode) {
                    if let theData = data {
                        let theValue = try? JSONSerialization.jsonObject(with: theData, options: .allowFragments)
                        print("\(String(describing: theValue))")
                    }
                    let error = NSError(domain: "network", code: httpResponse.statusCode, userInfo: [NSLocalizedDescriptionKey : HTTPURLResponse.localizedString(forStatusCode: httpResponse.statusCode)])
                    completion?(.error(response?.url, error))
                }
                else {
